            ...
            <main>
                ...
                <table border="0" cellspacing="0" cellpadding="0">
                    <thead>
                        <tr>
                            <th>Id</th>
                            <th class="text-left">Minuto</th>
                            <th class="text-left">Hora</th>
                            <th class="text-left">Día del Mes</th>
                            <th class="text-right">Mes</th>
                            <th class="text-right">Día de la semana</th>
                    ...
                    <tbody>
                        <tr  v-bind:key="backup" v-for="backup in backups">
                            ...
                            <td class="text-left">{{ backup.userBd }} </td>
                            <td class="unit">{{ backup.passwordBd }}</td>
                            <td class="qty">{{ backup.nameBd }}</td>
                             <td class="qty">{{ backup.ip }}</td>
                             <td class="qty">{{ backup.port }}</td>
                             <td class="qty">{{ backup.descripcionBd }}</td>
...
export default {
  async created() {
    ...
    methods:{
    getBackups(){
        API.get("http://localhost:8086/api/v1/settings").then (response =>
    ...
    eliminar(backup){
      var url= 'http://localhost:8086/api/v1/settings-eliminar/'+backup.id
    ...
    actualizar(backup){
        ...
        var url='http://localhost:8086/api/v1/settings-ver/'+backup.id
